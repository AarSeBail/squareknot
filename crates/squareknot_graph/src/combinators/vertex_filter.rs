...
pub struct VertexFilter<G: ViewCombinator, F: Fn(&G::VertexLabel) -> bool> {
    preimage: G,
    f: F,
}
...
impl<G: ViewCombinator, F: Fn(&G::VertexLabel) -> bool> ViewCombinator for VertexFilter<G, F> {
    type VertexLabel = G::VertexLabel;

    fn vertex_iterator<'a>(&'a self) -> impl Iterator<Item = Self::VertexLabel> + 'a {
        self.preimage.vertex_iterator().filter(|&v| (self.f)(&v))
    }
    ...
    fn neighbor_iterator<'a>(
        ...
        if (self.f)(&vertex) {
            self.preimage
                ...
                .map(|neigh| neigh.filter(|v| (self.f)(v)))
        } else {
            None
        }
    }
